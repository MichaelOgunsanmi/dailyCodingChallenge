        ...
        for i in range(len(rooms)):
            ...
            for key in rooms[i]:
                if key not in keysMap:
                    pickUpKeys(key, keysMap, rooms)
                keysMap[key] = True

            if len(keysMap) == len(rooms):
                return True

        return len(keysMap) == len(rooms)


def pickUpKeys(key, keysMap, rooms):
    for opening in rooms[key]:
        if opening not in keysMap:
            keysMap[opening] = True
            pickUpKeys(opening, keysMap, rooms)
