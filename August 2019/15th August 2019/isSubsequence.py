...
class Solution:
    def isSubsequence(self, testString: str, mainString: str) -> bool:
        if len(testString) == 0:
            return True
        ...
        while mainPointer < len(mainString):
            if testString[testPointer] == mainString[mainPointer]:
                testPointer += 1

            if testPointer == len(testString):
                return True
