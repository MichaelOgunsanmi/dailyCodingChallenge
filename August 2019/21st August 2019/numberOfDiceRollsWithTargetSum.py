...
class Solution:
    def numRollsToTarget(self, numOfDice: int, faces: int, target: int) -> int:
        ...
        if target < numOfDice or target > numOfDice * faces:
            return 0
        elif target == numOfDice or target == numOfDice * faces:
            return 1

        def helper(numOfDice, target):
            if numOfDice == 0:
            ...
            for targets in range(max(0, target - faces), target):
                value += helper(numOfDice - 1, targets)

            dp[(numOfDice, target)] = value
            return value

        return helper(numOfDice, target) % (10 ** 9 + 7)
