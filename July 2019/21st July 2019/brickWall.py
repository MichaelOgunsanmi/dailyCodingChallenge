...
# Note:
#
# The width sum of bricks in different rows are the same and won't exceed INT_MAX. The number of bricks in each row
# is in range [1,10,000]. The height of wall is in range [1,10,000]. Total number of bricks of the wall won't exceed
# 20,000.
...
class Solution:
    def leastBricks(self, walls: List[List[int]]) -> int:
        hashMap = {}
        totalWidth = sum(walls[0])

        for wall in walls:
            currentSum = 0
            for index, num in enumerate(wall):
                currentSum += num
