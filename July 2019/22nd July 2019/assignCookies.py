...
class Solution:
    def findContentChildren(self, greed: List[int], size: List[int]) -> int:
        contentChildren = 0
        if len(size) == 0 or len(greed) == 0:
            return contentChildren
        greed.sort()
        size.sort()

        greedPointer = len(greed) - 1
        sizePointer = len(size) - 1

        while greedPointer > -1 and sizePointer > -1:
            if size[sizePointer] >= greed[greedPointer]:
                contentChildren += 1
                greedPointer -= 1
                sizePointer -= 1
                continue
